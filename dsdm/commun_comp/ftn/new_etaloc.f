...
C 
      INTEGER IUNSIG
      INTEGER IUADMC, LONADM, LONSCH, IUADMN, IUADMI
      INTEGER LONPLA, LONEND, LONENP
...
C 
      INTEGER DMODUL, MODUL, DENDOM, ENDOM, LONRES
      INTEGER ADEND1
      LOGICAL NOCVD
...
C 
      DOUBLE PRECISION  EPSAR0(6), SIGAR0(6), EPSCR0(6), SIGCR0(6)
C  
...
C     ENDOM  endommagement d113, d213 et d212              X          3*(NPICET+1)
C     MODUL  modules E11AC(Y) et E22AC(Y) et E33AC(Y)      X   3*(NPICET+1)*YINTEG
C     ADEND1 endomagement sens fibre                       X          (NPICET+1)
C                                                             __________________________
C 
C                                                         YINTEG*(53*NPICET+9)+4*(NPICET+1)
C 
      LONPRO = YINTEG*(53*NPICET+9)+4*(NPICET+1) 
C 
...
C 
      ...
      DADEND = DMODUL + 3*(NPICET+1)*YINTEG
C 
...
C Quantites chapeau dans la base d'orthotropie stockee (en Entree) 
     &               DM(ENDOM), DM(MODUL), CASIN, DM(DCRICO),
     &               DM(ADEND1), DM(EPLAO), YDPIN,
C Quantites chapeau dans la base d'orthotropie stockee (en Sortie) 
     &               DM(ENDOM+3), DM(MODUL+3*YINTEG), CASOUT,
     &               DM(DCRICO+1),
     &               DM(ADEND1+1), DM(EPLAO+6*YINTEG), YDPOUT,
C Quantites erreur dans la base d'orthotropie (en Sortie) seront tournees apres incoco     
     ...
     &               NUMERR, DENOMI, LOGIMP, LOGFIB, RUPPRE)
C 
CD 		CALL IMPTDT ('VERIF ENDOM ', DM(ENDOM+3), 1, 3)
...
C 
                IF (LVISU .AND. LTYP3) THEN
                  IF (LOGR0) THEN
                    CALL QBORTH (-ANGORT, DM(EPSAOR), EPSAR0)
                    CALL QBORTH (-ANGORT, DM(SIGAOR), SIGAR0)
                    CALL QBORTH (-ANGORT, DM(EPSCOR), EPSCR0)
                    CALL QBORTH (-ANGORT, DM(SIGCOR), SIGCR0)

C-steph    		    EPSAR0 = EPSAR0+((INDI33-1)*6)
C-steph   		    SIGAR0 = SIGAR0+((INDI33-1)*6)
C-steph   		    EPSCR0 = EPSCR0+((INDI33-1)*6)
C-steph   		    SIGCR0 = SIGCR0+((INDI33-1)*6)
C-steph   		    EPLAO  = EPLAO+((INDI33-1)*6)
C-steph		    DDMODU = MODUL+((INDI33-1)*3)
		    CALL SAUVIC (TEMPS, EPSAR0, SIGAR0, EPSCR0, SIGCR0,
...
C     ENDOM endommagement d1, d2, d3                                    X 3*(NPICET+1)
C 
...
C 
                  LVISU = .TRUE.
C 
C                 DR  Modification du reperage du Point de Gauss le 11/1/96
C 
...
C 
C                 Si l'endommagement vaut 1 => c'est resolu
C 
                  DM(ENDOM+3) = DM(ENDOM)
                  NOCVD1    = .FALSE.
                END IF
C 
                IF (DM(ENDOM+1) .EQ. 1.D0) THEN
C 
C                 Si l'endommagement vaut 1 => c'est resolu
C 
                  DM(ENDOM+4) = DM(ENDOM+1)
                  NOCVD2    = .FALSE.
                END IF
C 
                IF (DM(ENDOM+2) .EQ. 1.D0) THEN
C 
C                 Si l'endommagement vaut 1 => c'est resolu
C 
                  DM(ENDOM+5) = DM(ENDOM+2)
                  NOCVD = .FALSE.
                END IF
C 
...
C 
	        SAUEOR = SAUEOR +3
                SAUPOR = SAUPOR +2
C 
C               DR Ajout le 9/7/96 : Option possible
C               DR On laisse l'endommagement precedent quand la contrainte normale
C                  a l'interface est negative
C 
C               REMODIF OA ET DL le 25/07/96
C 
C                IF (DM(SAUEOR+2) .GT. 0.D0) THEN
C 
C                  YAC  =  (G1*K1*DM(SAUEOR)*DM(SAUEOR)
C     &                 +  G2*K2*DM(SAUEOR+1)*DM(SAUEOR+1)
C     &                 +  K0*DMAX1(DM(SAUEOR+2), 0.D0)
C     &                    *DMAX1(DM(SAUEOR+2), 0.D0))/2.D0
C 
C                ATTENTION NOUVEAU MODELE DE L'INTERFACE :
C              2                      2
C     Yd1 = s13 / 2.k10  ;  Yd2  = s23 / 2.k20
C                     2
C     Yd3 = sup(0,s33) / 2.k0
C 
C                    alpha             alpha    alpha
C     Y  = (GAM1.Yd1)      + (GAM2.Yd2)     + Yd
C 
C           (1/alpha)
C     Y =  Y
C                           2
C     Y  =  Y - a.inf(s33,0) / 2.k0
C 
C     Y  =  sup(0,Y)
C     .                                 m       n
C     D = k.(  ((m/m+1)*(Y-Y0)/(Yc-Y0))  -  d )
C 
C     d1 = d2 = d
C 
                INVALP = 1.D0/ALPINT
CD              YD1 = 0.5D0*K1*DM(SAUEOR)*DM(SAUEOR)
CD              YD2 = 0.5D0*K2*DM(SAUEOR+1)*DM(SAUEOR+1)
                YD1 = 0.D0
                YD2 = 0.D0
...
CD              SINOR(1)  =  K1*DM(SAUEOR)
CD              SINOR(2)  =  K2*DM(SAUEOR+1)
C 
...
C 
CD              CALL IMPTDT ('VERIF SIGCH ', DM(BSGPAC), 1, LONSCH)
...
C 
      END IF
      CALL IMPDT ('YACAMAX ', YACAMAX)
      CALL IMPDT ('YACMAX  ', YACMAX)
C 
      IF (LTYP01) THEN
        CALL FERFIC (3, IUADMC, IDPROG)
        CALL FERFIC (3, IUNSIG, IDPROG)
...
C -----------------------------------------------------------------------
C 
C     Cette routine range les erreurs pour l'etape globale dans un tableau :
C 
C          NU-ERG-//NUETGL(NPICET+1)
C          DE-ERG-//NUETGL(NPICET+1)
C          ERGLOB-//NUETGL(NPICET+1)
C 
      SUBROUTINE ECERGL (NUMERR, DENERR, ERRGLO)
C 
C -----------------------------------------------------------------------
C 
C     DECLARATION DES PARAMETRES GLOBAUX
...
C 
      DOUBLE PRECISION NUMERR(NPICET+1)
      DOUBLE PRECISION DENERR(NPICET+1)
      DOUBLE PRECISION ERRGLO(NPICET+1)
C 
C -----------------------------------------------------------------------
C  
C     DECLARATION DES PARAMETRES LOCAUX
...
C 
      CHARACTER*3 CARETG
C 
      INTEGER   J, ADEPS, ADFEPS
C 
      CHARACTER*6 IDPROG
      PARAMETER (IDPROG='ECERGL')
C 
CD     CALL WLKBCD (IDPROG)
C 
C -----------------------------------------------------------------------
      CALL IDENTI (NBETGL, CARETG)
      CALL GESTDP ('NU-ERG-'//CARETG, NPICET+1, ADFEPS)
C 
      ADEPS = ADFEPS
C 
      DO J = 1, NPICET+1
      ...
      END DO
C 
      CALL GESTDP ('DE-ERG-'//CARETG, NPICET+1, ADFEPS)
C 
      ADEPS = ADFEPS
C 
      DO J = 1, NPICET+1
      ...
      END DO
C 
      CALL GESTDP ('ERGLOB-'//CARETG, NPICET+1, ADFEPS)
C 
      ADEPS = ADFEPS
C 
      DO J = 1, NPICET+1
